...
def updateDataByTitle(questionData):
    ...
    for (s,sentence) in enumerate(questionData):
        ...
        for (w,word) in enumerate(sentence):
            if type(word) == list:
                ...
                if fs.find(tmps) != -1 or tmpCount > 1000:
    ...
    print("***************************")
    return questionData
def selectLeastWord(wordArr):
    wordTimes = []
    for w in wordArr:
        count = 0
        for line in wikiArr:
            count += line.count(w)
        wordTimes.append(count)
    i = wordTimes.index(min(wordTimes))
    return wordArr[i]
...
def moreUpdateDataByTitle(questionData, linkArr):
    ...
    print("***************************")
    return questionData
def selectLinkPart(linkPart, linkArr, verbWord, isLast):
    if isLast == 1:
    ...
    for (i,l) in enumerate(linkArr):
        if l < 0:
            wordFreq[i] = 9999999
    result = [linkPart[wordFreq.index(min(wordFreq))], min(resultArr)]
    print("******wordFreq*********")
    print(wordFreq)
    print("***************************")
    print("******pairFreq*********")
...
def selectLastLinkPart(linkPart, linkArr):
    wordFreq = [0 for j in range(len(linkPart))]
    pairFreq = []
    pairArr = []
    resultArr = []
        ...
        for linkPart in sentence:
            if type(linkPart) == list:
                ...
                if len(linkPart) <= 1:
                    ...
                    linkPartCoupleArr = selectLinkPart(linkPart, linkArr, verbWord, 1)
                    newSentence.append(linkPartCoupleArr[0])
                    freqResult.append(linkPartCoupleArr[1])
        freqResult.append(selectLastLinkPart(newSentence, linkArr)[1])
        lastFreqResultArr.append(min(freqResult))
    # print(questions)
    # fout = codecs.open(output_txt, 'w', 'utf-8')
    # timesData = []
    # for questionArrs in questions:
    #     timesWrr = []
    #     for qs in questionArrs:
    #         timesWrr.append([0,0,0])
    #     timesData.append(timesWrr)
    # for line in wikiArr:
    #     for (qi,questionArrs) in enumerate(questions):
    #         for (i,twoWords) in enumerate(questionArrs):
    #             for (wi,word) in enumerate(twoWords):
